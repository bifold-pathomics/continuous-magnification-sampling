...
def create_multimag_dataset_enhanced(
    ...
    for split in ["train", "val", "test"]:
        ...
        assert split_source.exists(), (
            f"Source split directory does not exist: {split_source}"
        )
        ...
        for class_folder in class_folders:
            ...
            for img_path in tqdm(image_files, desc=f"    {class_name}", leave=False):
                with Image.open(img_path) as img:
                    ...
                    if min_size_filter and (
                        img.width < min_size_filter or img.height < min_size_filter
                    ):
                    ...
                    patch.save(
                        output_file, compress_level=6
                    )  # 6 is standard I believe, just to be sure
...
def create_multiple_magnifications_enhanced(
    ...
    **kwargs,
    ...
    if ensure_all_mags:
        ...
        print(
            f"Minimum source size filter: {min_size}px (to ensure all magnifications)"
        )
    ...
    for mpp, stats in results.items():
        ...
        if stats["total_filtered"] > 0:
            print(
                f"  (Filtered: {stats['total_filtered']}, Skipped: {stats['total_skipped']})"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create BRACS multi-magnification dataset"
    )
    ...
    parser.add_argument(
        "--method", type=str, default="progressive", help="Downsampling method"
    )
    parser.add_argument("--patch-size", type=int, default=224, help="Patch size")
    parser.add_argument(
        "--mpps", type=float, nargs="+", default=[0.25, 0.375, 0.5, 0.75, 1.0, 1.5, 2.0]
    )
